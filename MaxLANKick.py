...
try:
    ...
    if os.geteuid() != 0:
        print("\n{}ОШИБКА: Должно выполняться с правами суперпользователя. Попробуйте еще раз с помощью sudo:\n\t{}$ sudo python3 MaxLANKick.py{}\n".format(RED, GREEN, END))
        os._exit(1)
except:
...
def shutdown():
    print('\n\n{}Завершение Работы'
          '\nСпасибо за использование скрипта!{}'.format(GREEN, END))
    os._exit(0)
...
try:
    ...
    print("\n{}ОШИБКА: Требования не были выполнены должным образом. Пожалуйста, ознакомьтесь с файлом README для получения инструкций по настройке.".format(RED))
    print("\n{}Если вы все еще не можете устранить эту ошибку, пожалуйста, отправьте сообщение о проблеме здесь\n\t{}https://github.com/KaliSecurityMax\n\n{}Детали: {}{}{}".format(RED, BLUE, RED, GREEN, str(sys.exc_info()[1]), END))
    os._exit(1)
...
def heading():
    ...
    """ + END + BLUE +
    '\n' + '{}Блокировка интернета для устройств в локальной сети{}'.format(YELLOW, RED, YELLOW, BLUE).center(98) +
    '\n' + 'Разработал: {0}KaliSecurityMax \n'.format(YELLOW, END).center(86))
...
def optionBanner():
    ...
    sleep(0.2)
    print('\t{}[{}1{}]{} Отключить только одно устройство'.format(YELLOW, RED, YELLOW, WHITE))
    sleep(0.2)
    print('\t{}[{}2{}]{} Отключить несколько устройств'.format(YELLOW, RED, YELLOW, WHITE))
    sleep(0.2)
    print('\t{}[{}3{}]{} Отключить все устройства'.format(YELLOW, RED, YELLOW, WHITE))
    sleep(0.2)
...
def runDebug():
    print("\n\n{}ПРЕДУПРЕЖДЕНИЕ! Произошла неизвестная ошибка, запускающая отладку...{}".format(RED, END))
    print(
    "{}Старт дебага... (Вопросы сюда 'https://github.com/KaliSecurityMax'){}".format(
        RED, END))
    try:
    ...
    try:
        ...
        if defaultInterfaceMac == "" or not defaultInterfaceMac:
            ...
            "\n{}ERROR: Default Interface MAC Address could not be obtained. Please enter MAC manually.{}\n".format(
                RED, END))
            header = ('{}MaxLANKick{}> {}Enter MAC Address {}(MM:MM:MM:SS:SS:SS): '.format(BLUE, WHITE, RED, END))
            return (input(header))
        else:
...
def getGatewayIP():
    ...
    try:
        ...
        stopAnimation = True
        print("\n{}Внимание: Ведите IP вашей локальной сети{}\n".format(RED, END))
        header = ('{}MaxLANKick{}> {}Введите IP {}(Пример: 192.168.1.1): '.format(BLUE, WHITE, RED, END))
        return (input(header))
...
def regenOnlineIPs():
    ...
    if not defaultGatewayMacSet and defaultGatewayMac == "":
        ...
        header = ("{}MaxLANKick{}> {}Enter your gateway's MAC Address {}(MM:MM:MM:SS:SS:SS): ".format(BLUE, WHITE, RED, END))
...
def nonInteractiveAttack():
    ...
    print("\n{}nonInteractiveAttack{} включение...{}\n".format(RED, GREEN, END))

    target = options.targets
    print("\n{}Цели: {}{}".format(GREEN, END, ", ".join(target)))
    global stopAnimation
    stopAnimation = False
    t = threading.Thread(target=scanningAnimation, args=('Проверка...',))
    t.daemon = True
    t.start()
                ...
                for k, v in a['scan'].items():
                    if str(v['status']['state']) == 'up':
                        ...
                        if len(target) == 1 or counter == len(target)-1:
                            ...
                            print("\n{}Ошибка: Target {}{}{} doesn't seem to be alive. Exiting...{}".format(RED, END, str(host), RED, END))
                            os._exit(1)
                        else:
                            sys.stdout.write("\033[K")
                            print("\n{}Ошибка: Target {}{}{} doesn't seem be alive. Skipping...{}".format(RED, END, str(host), RED, END))
                            target.remove(host)
                            counter += 1
                            pass
            else:
...
def kickoneoff():
    ...
    global stopAnimation
    stopAnimation = False
    t = threading.Thread(target=scanningAnimation, args=('Сканирование...',))
    t.daemon = True
    t.start()
    ...
    while not canBreak:
        try:
            ...
            print("\n{}Ошибка: Выберите число из списка!{}".format(RED, END))
    ...
    for host in hostsList:
        if host[0] == oneTargetIP:
            oneTargetMAC = host[1]
    if oneTargetMAC == "":
        print("\nIP-адрес не указан. Пожалуйста, попробуйте снова.")
        return

    print("\n{}Цель: {}{}".format(GREEN, END, oneTargetIP))

    if options.packets is not None:
        print("\n{}Spoofing включен... {}( {} пакетов/мин )".format(GREEN, END, str(options.packets)))
    else:
        print("\n{}Spoofing включен... {}".format(GREEN, END))
    try:
        while True:
            # broadcast malicious ARP packets
...
def kickalloff():
    ...
    if options.packets is not None:
        print("\n{}Spoofing включен... {}( {} pkts/min )".format(GREEN, END, str(options.packets)))
    else:
        print("\n{}Spoofing включен... {}".format(GREEN, END))
    try:
        # broadcast malicious ARP packets
...
def main():
    ...
    if interactive:
        ...
        print("\n{}Используется интерфейс '{}{}{}' MAC Адрес: '{}{}{}'.\nIP: '{}{}{}' --> {}{}{} устройств обнаружено{}".format(
        ...
        print("\n{}Используется интерфейс '{}{}{}' и MAC адрес '{}{}{}'.\n IP: '{}{}{}' --> Target(s): '{}{}{}'.{}".format(
    ...
    if options.targets is None and options.scan is False:
        try:
            ...
            while True:
                ...
                header = ('{}MaxLANKick{}> {}'.format(BLUE, WHITE, END))
                choice = input(header)
        ...
        stopAnimation = False
        t = threading.Thread(target=scanningAnimation, args=('Сканирование сети...',))
        t.daemon = True
        t.start()
...
if __name__ == '__main__':
    ...
    version = '2.0'
    examples = ('\Примеры:\n'+
                '  sudo python3 MaxLANKick.py --target 192.168.1.10 \n'+
                '  sudo python3 MaxLANKick.py -t 192.168.1.5,192.168.1.10 -p 30\n'+
                '  sudo python3 MaxLANKick.py -s\n'+
                '  sudo python3 MaxLANKick.py (интерактивный режим)\n')

    parser = optparse.OptionParser(epilog=examples,
        usage='sudo python3 %prog [options]',
        prog='MaxLANKick.py', version=('MaxLANKick ' + version))
    ...
    try:
        if checkInternetConnection():
            ...
            print("\n{}Ошибка: It seems that you are offline. Please check your internet connection.{}\n".format(RED, END))
            os._exit(1)
    except KeyboardInterrupt:
        shutdown()
    ...
    if (options.packets is not None and (options.packets).isdigit()) or options.packets is None:
        ...
        print("\n{}Ошибка: Argument for number of packets broadcasted per minute must be an integer {}(e.g. {}--packet 60{}).\n".format(RED, END, BLUE, END))
        os._exit(1)
    ...
    if options.targets is None and options.kick_all is False:
        ...
        print("\n{}Ошибка: Cannot use both {}-a/--kick-all{} and {}-t/--target{} flags in one command.{}\n".format(RED, BLUE, RED, BLUE, RED, END))
        os._exit(1)
    else:
        # set to non-interactive attack
        interactive = False
